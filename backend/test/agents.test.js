...
vi.mock("../src/lib/contract.js", async (importOriginal) => {
  ...
  return {
    ...
    updatePolicyOnChain: vi.fn(), // ✅ Added mock for new function
  };
});
...
vi.mock("../src/config.js", () => ({
  default: {
    ...
    server: {
      secret: "SDY6B5V7K5L5K5L5K5L5K5L5K5L5K5L5K5L5K5L5K5L5K5L5K5L5K5L5K5L5K5L5"
    }
  },
}));
...
describe("POST /api/agents/register", () => {
  ...
  it("registers successfully and applies policy if agent is not registered", async () => {
    ...
    const payload = {
      ...
      maxPerTxUsdc: 100,
      maxPerDayUsdc: 1000,
      allowedCategories: ["payments", "transfers"],
    };

    const response = await request(app).post("/api/agents/register").send(payload);
    ...
    expect(contract.updatePolicyOnChain).toHaveBeenCalledWith(payload.agentAddress, {
      maxPerTxUsdc: payload.maxPerTxUsdc,
      maxPerDayUsdc: payload.maxPerDayUsdc,
...
describe("GET /api/agents/:address/payment-history", () => {
  ...
  it("returns 400 when pagination params are invalid", async () => {
    ...
    expect(res.body.errors).toContain("`limit` must be a positive integer");
  });
});
