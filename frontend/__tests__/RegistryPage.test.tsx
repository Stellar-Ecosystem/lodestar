...
/** Wraps component in a fresh SWR provider so each test gets an isolated cache */
    ...
    await waitFor(() =>
      expect(
        screen.getByText((_, el) => {
          ...
          return el?.tagName === 'P' && t.includes('Showing') && t.includes('1') && t.includes(`${PAGE_SIZE}`) && t.includes(`${total}`);
        })
      ).toBeInTheDocument()
    );
  });
